...
class VisualGridHuntGame:
    ...
    def execute_action(self, action: str):
        ...
        # Food interaction (+20 points)
        if tuple_pos in self.food_positions:
            self.food_positions.remove(tuple_pos)
            self.score += 20

        # Toxic trap interaction (-15 points)
        if tuple_pos in self.toxic_traps:
            self.score -= 15

        # Opponent interaction (-50 points and collision flag)
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        # Draw toxic traps (purple rectangles)
...
if __name__ == "__main__":
    root = tk.Tk()
    # Initializing GUI with a 12x12 grid, 15 food, 2 opponents, and 4 toxic traps
